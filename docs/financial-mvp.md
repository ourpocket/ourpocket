...
The TypeScript SDK is a separate private source package at `../../packages/typescript`, outside the frontend repository. It validates normalized API responses and types customers, providers, integrations, payments, refunds, transactions, sandbox wallets/transfers, and events. It supports base URL and request ID configuration and requires caller-supplied idempotency keys. It sends each financial write once. See its README for the runnable Sandbox and signed webhook receiver quickstart. The dashboard calls the API directly and does not require the SDK package to build.
