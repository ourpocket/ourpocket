...
5. Select Production in the dashboard, connect Paystack or Flutterwave for payments and Turnkey or Privy for wallets, generate an `op_live_sk_` key, and create a separate server-side client. Fake funding, simulated transfers, and scenarios are rejected in Production. Flutterwave checkout requires an HTTPS `callbackUrl`. Initial checkout is pending; use `payments.verify(id)` after checkout. Use `routing.get()` to inspect policy and `reconciliation.run()` to resolve pending or unknown operations against their original provider. This quickstart deliberately executes only sandbox operations.
